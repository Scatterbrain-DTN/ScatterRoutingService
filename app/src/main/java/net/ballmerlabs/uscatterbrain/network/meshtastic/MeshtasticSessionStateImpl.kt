        ...
        return mapStagePublisher(Stage.SYNACK) { s ->
            ...
            when (packet.code) {
                MeshtasticAckCode.FULL -> {
                    ...
                    Flowable.just(MeshtasticAnnounceSynAckPacket(code)).map { v -> v as ScatterSerializable<*> }.mergeWith(handleMerkleStream())
                }
            }
        }
    }
            ...
            datastore.getMerkleHubs(
                ...
                val obs: Flowable<ScatterSerializable<*>>  =
                    hubresponse.exclude.toList().flatMapPublisher { hashes ->
                        ...
                        val send = datastore.getTopRandomMessages(50, hashes)
